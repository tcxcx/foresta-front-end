...
export default function Button({ text, icon, onClick, disabled }: ButtonProps) {
    return (
        ...
        <SpotlightButton onClick={onClick} disabled={disabled}>
            ...
            {text && (
                <span className={`relative mt-px uppercase group-hover:text-basement-tone-purple bg-gradient-to-b from-white/25 to-white bg-clip-text font-clash text-lg  text-transparent transition-all duration-200 ${disabled ? 'disabled-styles' : ''}`}>
                    {text}
                </span>
            )}
        </SpotlightButton>
