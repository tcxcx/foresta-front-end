...
export async function getAvailableNFTs(collectionId: number) {
  ...
  const result = await api.query.communityProject.listedNftsOfCollection(
    collectionId
  );
...
export async function getAvailableNFTsbyType(
  collectionId: number,
  nftType: number
) {
  const api = await apiPromise;
  const result = await api.query.communityProject.listedNftTypes(
    collectionId,
    nftType
  );
  const output = result.toHuman();
